...
// MARK: - AcademicCalendarViewModel
/// Manages academic calendar state and logic: terms, events, grading periods,
/// report cards, and comment templates.
/// Extracted from AppViewModel+AcademicCalendar.swift to reduce god-class complexity.
...
class AcademicCalendarViewModel {
    ...
    /// Report card comments for students.
    var reportCardComments: [ReportCardComment] = []

    // MARK: - Calendar Config Access

    /// The resolved academic calendar configuration (demo data as fallback).
    var academicCalendarConfig: AcademicCalendarConfig {
        calendarConfig ?? Self.demoAcademicCalendar
    }

    // MARK: - Load Academic Calendar

    func loadAcademicCalendar(isDemoMode: Bool) async {
        if isDemoMode {
            calendarConfig = Self.demoAcademicCalendar
            return
        }
    ...
    func addTerm(_ term: AcademicTerm) {
        var config = calendarConfig ?? AcademicCalendarConfig()
    ...
    func generateReportCard(
        ...
        let attendanceSummary = ReportCardAttendance(
            presentCount: attendance.filter { $0.status == .present }.count,
            absentCount: attendance.filter { $0.status == .absent }.count,
            tardyCount: attendance.filter { $0.status == .tardy }.count,
            excusedCount: attendance.filter { $0.status == .excused }.count
        )
    ...
    func generateAllReportCards(
        ...
        if studentUsers.isEmpty, let current = currentUser {
            if let card = generateReportCard(
                for: current.id,
                termId: termId,
                grades: grades,
                courses: courses,
        ...
        return studentUsers.compactMap {
            generateReportCard(
                for: $0.id,
                termId: termId,
                grades: grades,
